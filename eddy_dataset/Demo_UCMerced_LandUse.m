...
addpath('F:/dudu/Matlab/PCANet_demo/Utils');
addpath('F:/dudu/Matlab/PCANet_demo/Liblinear');

TrnSize = 2100; 
ImgSize = 256; 

%% Loading data from CIFAR10 (50000 training, 10000 testing) 
DataPath = 'F:\dudu\Matlab\PCANet_RSI_Data\90_10dataset';
% CIFAR10 dataset can be downloaded at http://www.cs.toronto.edu/~kriz/cifar.html

TrnLabels = [];
TrnData = [];
%for i = 1:10
    load(fullfile(DataPath,['Trn_eddy_100.mat']));
    TrnData = [TrnData, Trn_eddy'];
    TrnLabels = [TrnLabels; Trn_label];
%end
load(fullfile(DataPath,'Test_eddy_36.mat'));
TestData = Test_eddy';
